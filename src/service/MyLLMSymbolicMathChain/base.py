...
from src.service.MyLLMSymbolicMathChain.sympy_template import PROMPT
...
class LLMSymbolicMathChain(Chain):
    ...
    def _evaluate_expression(self, sympy_code: str) -> str:
        try:
            import sympy
        ...
        try:
            vars = {}
            exec(sympy_code, vars)
            output = str(vars['res'])
        except Exception as e:
            raise ValueError(
                f'LLMSymbolicMathChain._evaluate("{sympy_code}") raised error: {e}.'
                " Please try again with a valid sympy code"
            )
